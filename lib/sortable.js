var listMixin = {
  ...
  onMoveBefore: function(elem) {
    },
    onMoveEnd: function(elem) {
        setTimeout(function() {
            window.moving = false;
        }, 300);
  ...
  bindMove(movedComponent, moveElemEvent) {
    var moveElem = movedComponent.getDOMNode()
      ...
      , computedStyle = window.getComputedStyle(moveElem);
    ...
    // Keep the initialized position in DOM
      moveElem.style.left = (moveElemPosition.left - parentPosition.left) - parseInt(computedStyle.getPropertyValue('margin-left')) + 'px';
      moveElem.style.top = (moveElemPosition.top - parentPosition.top) - parseInt(computedStyle.getPropertyValue('margin-top')) + 'px';
  ...
  resort(oldPosition, newPosition) {
    var items, movedItem;
    if (oldPosition !== newPosition) {
      ...
      items.splice(newPosition, 0, movedItem);
        if (this.onBeforeSetState) {
            this.onBeforeSetState(items);
        }
...
sortable = {};
sortable.ItemMixin = itemMixin;
sortable.ListMixin = listMixin;
